...
router.post('/register', async (req, res, next) => {
    try {
        const isUser = await user.findOne({ $or: [{ email: req.body.email }, { username: req.body.username }] });
        if (isUser) return responseFunction(res, 201, 'User already exists', null, false);
...
router.post('/login', async (req, res, next) => {
    ...
    try {
        ...
        if (result) {
            ...
            if (isMatch) {
                if (result.isEmailVerified === false) return responseFunction(res, 201, 'Please verify your email first', null, false);
                result.password = undefined;
...
router.get('/get-user', async (req, res, next) => {
    ...
    try {
        ...
        return responseFunction(res, 200, 'User found', result, true);
    } catch (err) {
        return responseFunction(res, 400, 'Error sending OTP', err.message, false);
    }
});
...
router.post('/send-otp', async (req, res, next) => {
    ...
    try {
        const result = await user.findOne({ email });
        if (!result) return responseFunction(res, 400, 'User not found', null, false);
        if (result.isEmailVerified) return responseFunction(res, 200, 'Your email is already verified', null, false);
